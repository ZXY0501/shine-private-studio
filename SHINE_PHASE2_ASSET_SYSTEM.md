...
- 已完成：边框大类、同订单多边框叠加、置顶/置底、删除、四锚点吸色和手动微调。
- 已完成：单层微调只显示当前订单已经勾选的 A/B 素材；拖动/缩放/旋转改用轻量预览，松手后再精确重组。
- 已完成：纯净模板与素材 PSD 自动保存到 IndexedDB；刷新后自动恢复最近模板，素材只先恢复 metadata，点选时再解析 PSD。
- 已完成：素材位置、缩放、旋转和翻转按订单分别保存；切换订单不会串用上一单的位置。
- 已完成：云端目录优先复用匹配 `assetId + version + contentHash` 的本机缓存；纯净模板也可通过云素材接口保存、读取和删除。
- 待联调：Phase 2 FC 包和 OSS CORS 尚未部署，当前云素材按钮只属于隐藏测试入口。
...
边框是全局素材，不占用 A/B 单选位。同一订单可以同时加入多个边框：
...
- “置顶”表示放在人物和普通素材上方；
- 边框仍支持移动、缩放、旋转和翻转；
- 缎带 A、缎带 B、花卉/底框底色、花卉/底框重色是四个独立颜色锚点；
- 未设置锚点时保留 PSD 原色，吸色或手动选色后才改写对应 Role；
- 吸色按钮进入一次性吸色状态，下一次点击左侧成图画布即取样，使用邻域中位色降低点到线稿或抗锯齿边缘的误差。
...
白色、浅粉色帽子使用更深的低饱和灰调轮廓，避免成图过淡；该规则只作用于自动推导轮廓，固定方案中明确保存的线稿色仍然优先。
...
Windows 文件名不能使用半角 `*`，因此规范分隔符统一使用下划线 `_`。网页同时兼容空格、连字符和旧文件名，不要求为旧素材返工。

推荐文件名：

- `模板_模板名字.psd`
- `耳朵_动物名字.psd`（即使 PSD 同时含尾巴，也仍按耳朵套装管理）
- `头发_角色名字.psd`
- `嘴巴_表情名字.psd`
- `尾巴_动物名字.psd`
- `帽饰_帽饰名字.psd`
...
推荐顶层夹子：`A`、`B` 或 `A位`、`B位`。解析时人物位以顶层父夹子为最高优先级，所以现有模板中 B 夹子里误写成“A衣服底色”的子层仍会归到 B，不会被名字里的 A 抢走。

耳朵套装图层：

- `A线稿`、`A底色`、`A重色`、`A内耳`、`A绒毛`、`A阴影`
- `B线稿`、`B底色`、`B重色`、`B内耳`、`B绒毛`、`B阴影`
- 若同 PSD 含尾巴：`A尾巴线稿`、`A尾巴底色`、`A尾尖`，B 同理
- 若同 PSD 含鹿角/犄角：`A角线稿`、`A角底色`、`A角重色`，B 同理

纯净模板建议图层：

- 帽子：`A帽子线稿`、`A帽子底色`、`A帽子辅色`、`A帽子绒毛`，B 同理
- 衣服：`A衣服线稿`、`A衣服底色`、`A衣服辅色`、`A衣服阴影`，B 同理
- 眼睛：`A虹膜底色`、`A虹膜暗色`、`A虹膜深色`、`A虹膜高光`、`A瞳孔点缀`、`A瞳孔暗色`、`A瞳孔高光`、`A眼睛线稿`、`A睫毛`、`A睫毛高光`，B 同理
- 头发：`A头发底色`、`A头发重色`、`A头发线稿`、`A头发高光固定`、`A肤色喷枪固定`，B 同理
- 背景：`背景底色`、`背景蕾丝`、`水印`
...
- `花卉_底色`、`花卉_重色`
- 若是整块底框：`底框_底色`、`底框_重色`
- 不参与自动改色的文字、星点、高光等，在图层名中加入 `固定`

命名是自动识别提示，不是破坏性规则。最终仍可在“图层显示 / 颜色角色”中人工修正。
...
- 浏览器本机库包含四个独立 store：`assetSources`、`assetCatalog`、`templateSources`、`templateCatalog`；PSD Blob 与轻量 metadata 分开保存。
- 页面刷新时自动恢复最近使用的纯净模板；其他模板和所有素材只显示目录，实际使用时才解析，避免启动时一次性吃满内存。
- A/B 头发专用上传入口也写入同一素材库；上传新头发只切换当前订单选择，不再删除这个位置已经存过的其他头发。
